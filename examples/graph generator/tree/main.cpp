...
ofstream graph;
const int number_of_nodes=4095;
const int size_of_date=1000;
...
void printnode(int number, int from, int to)
{
    ...
    if (number == 1)
    {
        graph<<"   num_input_edges 0"<<endl;
        graph<<"   edges (  )"<<endl;
    }else{
        graph<<"   num_input_edges 1"<<endl;
        graph<<"   edges ( "<<from<<" )"<<endl;
    }
    if (to==0)
    {
...
void printedge(int number, int from, int to)
{
    ...
    graph<<"       right_offset \""<<size_of_date<<"\""<<endl;
